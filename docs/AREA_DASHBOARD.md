...
## Change Log
- 2026-06-11: Plan 039. Dashboard My Gear and Team Activity overflow footers now hide whenever any dashboard filter is active, including location-only filters, so scoped row counts no longer sit above unfiltered `View all` totals.
...
- 2026-06-10: **Plan 027 -- shadcn Item row migration**: Dashboard queue rows now route through the shared shadcn `Item` row primitive (`booking-row.tsx`, `my-gear-column.tsx`, `team-activity-column.tsx`). Booking selection, inline row actions, gear/avatar summaries, and operational badges (due, pickup, coverage) are fully preserved. My Shifts and Drafts rows use `Item`/`ItemGroup`/`ItemContent`/`ItemTitle`/`ItemDescription`/`ItemActions`. Upcoming event rows use `Item` with preserved left status rail, under-coverage red tint, `ShiftAvatarStack`, coverage badge, and venue badge.
