...
5. `xcodebuild` for `Wisconsin` on `generic/platform=iOS`
...
- Destination: the newest installed iPhone Simulator for main-app review. Use an iPadOS 17-compatible iPad destination for `WisconsinKiosk` checks.
...
- `npm run ios:xcode:verify` passes for the touched target.
