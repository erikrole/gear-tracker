...
## Change Log
- 2026-06-23: Battery Ops checked-out unit context now derives holder, booking, due date, and age from active `BookingBulkUnitAllocation` rows for `OPEN` checkout bookings, while keeping unit status counts sourced from `BulkSkuUnit.status`. For existing orphaned checked-out unit rows created without allocation records, the read model falls back to matching open unit-tracked checkout bulk items for the same SKU, capped by outstanding planned quantity, so valid booking context is visible without over-attaching stale units.
