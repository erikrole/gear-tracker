...
- Decision:
  ...
  - Store artifacts in a private Blob store under immutable ID-based paths. Download and preview go through authenticated routes; SVG is attachment-only. Collection-card Download All reads only current committed SVG revisions and returns a deterministic private ZIP with clean signer filenames and collision suffixes. Box and native capture remain deferred.
