...
- 2026-03-31: **Booking sheet deep links** — Checkout/reservation redirect pages now forward all URL search params (`create`, `newFor`, `title`, `startsAt`, `endsAt`, `locationId`, etc.) to `/bookings`. Dashboard "New checkout"/"New reservation" buttons and item detail "Check out"/"Reserve" buttons now deep-link straight to the CreateBookingSheet. Item-page deep links (`?newFor=assetId`) pre-select the asset and open the details section.
