...
| 2026-06-11 | Mobile filter sheet now includes a shadcn `SheetDescription` (sr-only) describing available filter dimensions; filter rail behavior and layout unchanged. |
| 2026-06-11 | Resource delete confirmation now uses `AlertDialogAction variant="destructive"` via the shared shadcn prop instead of manual destructive class strings. |
