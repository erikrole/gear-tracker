...
| GAP-65 | Signature capture rollout proof | AREA_SIGNATURES | Active | The V1 release is deployed to production as Vercel deployment `dpl_5WSeBG88rhaBNKKmtfM6ETVX75Um`; production reports 119/119 migrations applied, and unauthenticated `/signatures` correctly redirects to `/login`. The MBB pilot plus standalone Creative staff roster are live, with the corrected 14-player / 7-coach / 11-support-staff hierarchy, numeric player order, UWBadgers source-order staff presentation, safe archival of 2025–26, and the separate linked Creative staff collection sourced from 12 active full-time Video/Photo/Graphics accounts. Physical iPad Safari pen/touch proof, private Blob provisioning, authenticated capture/save proof, and cleanup failure-injection evidence remain. Source: `tasks/signature-capture-micro-app-plan.md`. |
