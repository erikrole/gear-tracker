...
| GAP-28 | React Query adoption incomplete — Kits page uses raw useState+useEffect, Search uses raw fetch(), Bookings delegates internally | CROSS-CUTTING | Expected | ~70% adoption via useFetch/custom hooks. Remaining 3 pages work fine but miss cache/dedup. Low priority. |
| GAP-29 | URL state not persisted on Kits and Bulk Inventory pages — filters lost on navigation | CROSS-CUTTING | Open | Add useUrlState to these 2 pages for filter persistence. Quick win. |
| GAP-30 | No CSP header — relies on React output escaping for XSS prevention | CROSS-CUTTING | Open | Add Content-Security-Policy header in next.config.ts for defense-in-depth. Low risk for internal app. |
| GAP-31 | User deactivation UI not wired — login blocking works but no admin toggle to set active=false | AREA_USERS | Open | `BRIEF_USER_DEACTIVATION_V1.md` exists. Login blocking shipped 2026-04-03. Needs: admin UI toggle, booking migration, visual indicator. |
