...
| GAP-63 | A forced shift delete removes a worker's assignment with no notification | AREA_SHIFTS | Active | Deleting a staffed shift with `?force=true` cancels open trades and removes the assignment, but sends nothing to the affected worker -- unlike `DELETE /api/shift-assignments/[id]`, which fires a `removed` schedule notification. `createShiftScheduleNotification` resolves an assignment by id, so notifying requires capturing the affected workers before the delete rather than after. Both delete routes are affected. Needs a product call on whether a forced delete should notify like a removal does. |
| GAP-64 | Sport-settings propagation has two conflicting rules for a shift's coverage window | AREA_SHIFTS / AREA_SETTINGS | Active | A grouped sport-settings save runs `rebaseUpcomingShiftsForSportCodes`, which only retimes a shift that is `templateManaged` with no assignments, notes, or explicit call window, and then runs `syncCurrentSportCallTimes`, which retimes every shift whose window differs from the sport default with no provenance check. The second step therefore overrides the first step's protection of manual work. A read-only audit on 2026-08-05 across 55 future timed events found 0 shifts currently drifting from their sport default, so this is latent rather than active -- the August call-time repair normalized everything. Resolving it needs a product decision: is `Shift.startsAt/endsAt` a settings-derived fallback (current sync behavior, and what the 2026-08-04 note describes) or protected manual intent (current generation behavior)? |
