...
| GAP-69 | Software Vault credential lifecycle and key rotation proof | AREA_SOFTWARE / AREA_LICENSES | Active | Production code, a production-only Sensitive key, migration `0125_software_credentials`, and authenticated desktop/narrow empty-state proof are complete. The first real administrator-entered credential still needs create/reveal/copy/archive/restore acceptance, and coordinated key rotation remains an operational procedure. No fabricated credential will be seeded for proof. Source: `docs/AREA_SOFTWARE.md` and `tasks/software-vault-plan-2026-08-19.md`. |
...
- 2026-08-19: Production deployment `dpl_BuEMXuk96yzqyjj9sPTTAPEAQ2tS` from commit `2548f4ce` closed the Software Vault code, key, migration, and authenticated empty-state gates. GAP-69 now tracks only first real-credential lifecycle acceptance and coordinated key rotation.
