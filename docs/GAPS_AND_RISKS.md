...
| GAP-69 | Software Vault rollout and key lifecycle proof | AREA_SOFTWARE / AREA_LICENSES | Active | Production code, a production-only Sensitive key, migration `0125_software_credentials`, and authenticated desktop/narrow empty-state proof are complete. The audience-gated follow-up adds migration `0126_software_credential_visibility`, collaborator-policy review, and role-scoped runtime proof; the first real administrator-entered credential still needs create/reveal/copy/archive/restore acceptance, and coordinated key rotation remains an operational procedure. No fabricated credential will be seeded for proof. Source: `docs/AREA_SOFTWARE.md` and `tasks/software-vault-plan-2026-08-19.md`. |
