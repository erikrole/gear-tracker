...
- 2026-08-20: Reconciled the macOS companion follow-up with local source and build hardening. Wisconsin Creative now uses event-driven external-projection refresh, durable pending revocation cleanup, validated bounded projections, privacy cleanup on sign-out, canonical count/health presentation, and Release Hardened Runtime configuration. Focused source contracts, Swift parsing, and build-for-testing pass locally; native test-runner communication, APNs, signed clean Release, installed-app, and visual acceptance remain open under the existing Companion delivery gap.
