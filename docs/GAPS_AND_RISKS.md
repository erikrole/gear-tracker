...
- 2026-03-25: P0 backlog sweep — Fixed dashboard reservation 7-day filter (AC-4). Cleaned notification icon type mapping (removed dead UPPER_CASE types). Fixed importer sourcePayload to store only unmapped CSV columns (D-014). Verified BulkSku routing works correctly. Confirmed export button already wired (AC-12). Added assetTag uniqueness check on create (onBlur validation). Documented Hobby plan cron constraint (once/day limit). Updated AREA_SETTINGS to reflect ADMIN+STAFF policy.
- 2026-03-25: P1 hardening sweep — Hardened 7 pages: notification center (skeleton, error state, pre-shadcn→Tailwind), categories (error state, inline styles→Tailwind), sports (error state, skeleton), escalation (data-table→shadcn Table, toggles→Switch), import page (full rewrite: data-table→shadcn, inline styles→Tailwind, hardcoded colors→dark-mode-safe, `<a>`→`<Link>`), item detail (inline styles→Tailwind). Verified BookingListPage and NewItemSheet already hardened.
