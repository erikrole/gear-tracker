...
| GAP-15 | Schedule page is 1,012 lines — monolithic, blocks feature additions | AREA_SHIFTS | Open | V2: decompose into hooks (`useScheduleData`) + leaf components (`CalendarView`, `ListView`, `ScheduleFilters`) |
...
| Monolithic page files | Schedule (1,012 lines) grows with each feature | V2: decompose schedule; scan decomposed 2026-03-25 (1,038→251 lines) | Engineering |
...
- 2026-03-25: Closed GAP-14 (scan page decomposed). `page.tsx` reduced from 1,038→251 lines (76% reduction). Extracted: `useScanSession` hook (status, polling, completion), `useScanSubmission` hook (scan processing, feedback, unit picker), 4 leaf components (`ScanControls`, `ScanChecklist`, `UnitPickerSheet`, `ItemPreviewSheet`), shared types module. Monolithic page files risk updated — only schedule remains.
