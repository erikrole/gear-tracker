...
| ~~GAP-31~~ | ~~User deactivation not fully wired~~ | ~~AREA_USERS~~ | ~~Closed~~ | ~~Login blocking, booking migration (block on OPEN checkouts, auto-cancel BOOKED/DRAFT), admin toggle, inactive badge all shipped. 2026-04-03. Session-level enforcement added 2026-04-06: `requireAuth()` checks `user.active`, deactivation deletes sessions.~~ |
...
| GAP-33 | PENDING_PICKUP bookings have no auto-expiry — orphans can accumulate if student never shows up | AREA_KIOSK | Expected | Staff/admin can cancel manually. Cron-based auto-expiry (e.g., 48h after startsAt) deferred to future sprint. |
...
- 2026-04-30: Platform-parity assessment. Added "Platform Parity (V1)" table to `AREA_MOBILE.md` documenting intentional web-only vs iOS-on-parity surfaces (Kits, Reports, Importer, Labels, Settings panels, Guides are deliberately web-only; Dashboard, Bookings, Items, Scan, Schedule, Search, Events, Notifications, Profile are at parity; Kiosk is iOS-canonical). GAP-34, GAP-35, GAP-36 confirmed Expected (intentional V1 deferrals). Audit-doc-truth pass: verified in source that the 2026-04-27 schedule Pass 2 P1s (isStale, workerTypeLabel, push routing), the 2026-04-24 kiosk cart-persistence P0, and the 2026-04-24 dashboard overdue-count P0 had all shipped — verdict notes added to the three audit docs. No new gaps, decisions, or risks opened.
