...
| GAP-65 | Signature capture rollout proof | AREA_SIGNATURES | Active | Production deployment `dpl_3rg2roHp8uDg8UqsGByJaKaUnQQK` from commit `16f6ea6e` is READY and aliased to `wisconsincreative.com`. The MBB and standalone Creative staff rosters remain live, and the open `VB / 2026-27` collection now reads back as 18 active players, 17 READY private revision-1 artifacts, one intentionally blank #16, an applied 31-entry source snapshot, 19 matching audit rows, and zero pending-delete revisions. The dedicated private store `wisconsin-creative-signatures-private` remains separate from public media. Football roster/artifact backfill is the next bounded slice. A real Pencil save, authenticated artifact delivery, and physical iPad Safari pen/touch/rotation/interruption proof remain. Source: `tasks/signature-capture-micro-app-plan.md`. |
...
- 2026-08-16: Added and deployed sport-aware Signature roster import support for Football (`FB`) and Volleyball (`VB`) in the `2026-27` season without a schema migration. Production Volleyball roster apply and the 17-artifact private Illustrator backfill are complete with #16 intentionally blank; Football remains a separately reviewed next slice under GAP-65.
