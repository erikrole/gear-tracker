...
| GAP-66 | Email-first invited onboarding rollout proof | AREA_USERS / AREA_MOBILE | Active | `/api/auth/discover`, the shared web login flow, native iOS discovery/registration wiring, the legacy `/register` redirect, and focused source/API tests are implemented locally. Authenticated web smoke must prove an unclaimed allowlist email creates the account and reaches Welcome without a registration-link handoff; the iPhone 16 Pro simulator build/source gates and a real installed app acceptance remain before production claims. Source: `tasks/email-first-onboarding-plan-2026-08-17.md`. |
