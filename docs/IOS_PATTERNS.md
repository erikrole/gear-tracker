...
> **Native tab shell.** `AppTabView` uses SwiftUI's built-in value-based `Tab(...)` API. The compact iPhone tab set is Home, Schedule, Bookings/My Gear, Browse, and Search. Compact iPhone uses `.tabBarOnly`; Search is tab value `3` with `role: .search` and pinned placement so the system owns the dedicated trailing search placement. The embedded `GlobalSearchSheet` binds `.searchable(..., isPresented:)` and presents the native search field on appear so tapping Search opens the keyboard without a second tap. Browse is tab value `2` and opens Items, Guides, Licenses, and Users with a native grouped-list menu. Regular-width layouts use `.sidebarAdaptable` plus sidebar-only Resources destinations for Guides, Licenses, and Users. Settings > Directory remains a fallback for secondary destinations. Do not stack a custom bottom bar over the native tab bar, and do not add a sixth compact tab that forces Search out of its trailing position.

> **Embedded navigation ownership.** A hub destination owns one `NavigationStack` and one bound path for the hierarchy it presents. Child list views that also appear standalone expose a `wrapsInNavigationStack` switch: standalone/sidebar entry points keep the default wrapper, while Browse passes `false` for Items, Guides, Licenses, and Users. Register child detail destinations on the embedded content so they participate in the ancestor stack. This prevents child loading toolbars from rendering in a second navigation layer, keeps push animations coherent, and makes Back return one level instead of escaping to the hub root.
