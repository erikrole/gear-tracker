...
- 2026-06-11: Audit Log table shadcn migration. Settings > Audit Log now renders its table through the shared shadcn `Table` primitive (`TableHeader`, `TableBody`, `TableHead`, `TableRow`, `TableCell`), replacing hand-rolled markup. Filters, pagination, auto-refresh, and row content are unchanged.
- 2026-06-11: Sports minimum-crew matrix now uses shared shadcn Table primitive. `ShiftConfigTable` replaces the hand-rolled table markup with shadcn Table components. The two-level area/worker-type header and all home/away input controls are preserved with their original `id`, `name`, `aria-label`, disabled state, and update handlers.
