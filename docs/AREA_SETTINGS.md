...
## Change Log
- 2026-06-24: **Hidden smoke user visibility.** Added `User.hiddenFromRoster` so smoke/test identities can remain active for verification while staying out of default roster, export, form-option picker, kiosk user-selection, and non-internal profile reads. Internal operator opt-in is controlled by `INTERNAL_OPERATOR_EMAILS`; `/api/me` exposes that current-user capability, `/users` shows an owner-only "Show hidden test users" filter that also carries into roster CSV export, and `POST /api/users/hidden-cleanup` lets internal operators dry-run or apply age-based hidden-user deactivation. Normal staff/admin access still excludes hidden users by default.
