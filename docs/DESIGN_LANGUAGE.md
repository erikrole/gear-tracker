...
- Web sidebar: Lookup is now a first-class `/scan` destination in the sidebar, Settings is visible to every authenticated role, Bookings uses overdue-first then user-scoped due-today badge priority, and collapsed sidebar tooltips include shortcut hints.
