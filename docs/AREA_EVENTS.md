...
21. **Crew operations cleanup** — Schedule event-row menus own Home, Away, or empty crew setup. Event detail Crew owns staff/admin assignment, replacement, slot, call-window, and timed-release management through the versioned working copy; workers keep a read-only crew table. Template-review and attendance controls are not surfaced.
