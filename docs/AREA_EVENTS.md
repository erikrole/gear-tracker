...
21. **Crew template and copy-forward review** — Event detail Crew exposes a staff/admin review flow for recurring events. It compares current crew slots to the active sport template, calls out additive missing slots and extra/manual slots, previews copy-forward assignments from the last matching staffed event, and applies copied crew through the existing assignment safety checks instead of silently mutating the schedule.
