...
24. **Source-backed game results** — Synced event titles may contribute only a leading `[W]` or `[L]` marker to nullable `CalendarEvent.result`. Sync extracts the result before display-title cleanup, keeps the complete upstream title in `rawSummary`, treats corrections and marker removal as real changes, and never infers a result for manual events.
...
## Change Log
- 2026-07-23: **Source-backed W-L results.** Calendar sync now extracts a leading `[W]` or `[L]` into nullable event result data before cleaning Wisconsin title boilerplate. The exact source title remains in `rawSummary`; result-only corrections enter the daily change digest, and removing a recognized marker clears the stored result. Migration `0106_calendar_event_results` backfills only imported events with captured raw evidence. Historical events absent from both stored evidence and the current source remain unknown rather than receiving manufactured outcomes.
