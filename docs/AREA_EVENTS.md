...
## Change Log
- 2026-06-16: Schedule and event detail all-day display cleanup shipped. All-day event labels now keep the date as the canonical event timing and suppress inherited local midnight-to-midnight default crew call windows, so manual all-day events no longer show duplicate `12:00 AM` ranges while explicit slot or personal call-time overrides remain visible.
