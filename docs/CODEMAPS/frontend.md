<!-- Generated by scripts/generate-codemaps.mjs. Do not edit by hand. -->
# Frontend Map

## Pages

| Route | File |
|---|---|
| `/admin/fix-today` | `src/app/(app)/admin/fix-today/page.tsx` |
| `/bookings` | `src/app/(app)/bookings/page.tsx` |
| `/bulk-inventory/[id]` | `src/app/(app)/bulk-inventory/[id]/page.tsx` |
| `/bulk-inventory/batteries` | `src/app/(app)/bulk-inventory/batteries/page.tsx` |
| `/bulk-inventory` | `src/app/(app)/bulk-inventory/page.tsx` |
| `/checkouts/[id]` | `src/app/(app)/checkouts/[id]/page.tsx` |
| `/checkouts/new` | `src/app/(app)/checkouts/new/page.tsx` |
| `/checkouts` | `src/app/(app)/checkouts/page.tsx` |
| `/events/[id]` | `src/app/(app)/events/[id]/page.tsx` |
| `/events` | `src/app/(app)/events/page.tsx` |
| `/import` | `src/app/(app)/import/page.tsx` |
| `/items/[id]` | `src/app/(app)/items/[id]/page.tsx` |
| `/items/hygiene` | `src/app/(app)/items/hygiene/page.tsx` |
| `/items` | `src/app/(app)/items/page.tsx` |
| `/kits/[id]` | `src/app/(app)/kits/[id]/page.tsx` |
| `/kits` | `src/app/(app)/kits/page.tsx` |
| `/labels` | `src/app/(app)/labels/page.tsx` |
| `/licenses` | `src/app/(app)/licenses/page.tsx` |
| `/notifications` | `src/app/(app)/notifications/page.tsx` |
| `/` | `src/app/(app)/page.tsx` |
| `/profile` | `src/app/(app)/profile/page.tsx` |
| `/reports/audit` | `src/app/(app)/reports/audit/page.tsx` |
| `/reports/badges` | `src/app/(app)/reports/badges/page.tsx` |
| `/reports/bulk-losses` | `src/app/(app)/reports/bulk-losses/page.tsx` |
| `/reports/checkouts` | `src/app/(app)/reports/checkouts/page.tsx` |
| `/reports/overdue` | `src/app/(app)/reports/overdue/page.tsx` |
| `/reports` | `src/app/(app)/reports/page.tsx` |
| `/reports/scans` | `src/app/(app)/reports/scans/page.tsx` |
| `/reports/utilization` | `src/app/(app)/reports/utilization/page.tsx` |
| `/reservations/[id]` | `src/app/(app)/reservations/[id]/page.tsx` |
| `/reservations/new` | `src/app/(app)/reservations/new/page.tsx` |
| `/reservations` | `src/app/(app)/reservations/page.tsx` |
| `/resources/[slug]/edit` | `src/app/(app)/resources/[slug]/edit/page.tsx` |
| `/resources/[slug]` | `src/app/(app)/resources/[slug]/page.tsx` |
| `/resources/new` | `src/app/(app)/resources/new/page.tsx` |
| `/resources` | `src/app/(app)/resources/page.tsx` |
| `/scan` | `src/app/(app)/scan/page.tsx` |
| `/schedule/assign` | `src/app/(app)/schedule/assign/page.tsx` |
| `/schedule` | `src/app/(app)/schedule/page.tsx` |
| `/search` | `src/app/(app)/search/page.tsx` |
| `/settings/allowed-emails` | `src/app/(app)/settings/allowed-emails/page.tsx` |
| `/settings/appearance` | `src/app/(app)/settings/appearance/page.tsx` |
| `/settings/audit` | `src/app/(app)/settings/audit/page.tsx` |
| `/settings/bookings` | `src/app/(app)/settings/bookings/page.tsx` |
| `/settings/calendar-sources` | `src/app/(app)/settings/calendar-sources/page.tsx` |
| `/settings/categories` | `src/app/(app)/settings/categories/page.tsx` |
| `/settings/checkout-policies` | `src/app/(app)/settings/checkout-policies/page.tsx` |
| `/settings/data-export` | `src/app/(app)/settings/data-export/page.tsx` |
| `/settings/database` | `src/app/(app)/settings/database/page.tsx` |
| `/settings/departments` | `src/app/(app)/settings/departments/page.tsx` |
| `/settings/escalation` | `src/app/(app)/settings/escalation/page.tsx` |
| `/settings/kiosk-devices` | `src/app/(app)/settings/kiosk-devices/page.tsx` |
| `/settings/locations` | `src/app/(app)/settings/locations/page.tsx` |
| `/settings/notifications` | `src/app/(app)/settings/notifications/page.tsx` |
| `/settings` | `src/app/(app)/settings/page.tsx` |
| `/settings/profile` | `src/app/(app)/settings/profile/page.tsx` |
| `/settings/reservation-rules` | `src/app/(app)/settings/reservation-rules/page.tsx` |
| `/settings/security` | `src/app/(app)/settings/security/page.tsx` |
| `/settings/sports` | `src/app/(app)/settings/sports/page.tsx` |
| `/settings/venue-mappings` | `src/app/(app)/settings/venue-mappings/page.tsx` |
| `/users/[id]` | `src/app/(app)/users/[id]/page.tsx` |
| `/users/onboarding-status` | `src/app/(app)/users/onboarding-status/page.tsx` |
| `/users/org-chart` | `src/app/(app)/users/org-chart/page.tsx` |
| `/users` | `src/app/(app)/users/page.tsx` |
| `/change-password` | `src/app/change-password/page.tsx` |
| `/forgot-password` | `src/app/forgot-password/page.tsx` |
| `/login` | `src/app/login/page.tsx` |
| `/register` | `src/app/register/page.tsx` |
| `/reset-password` | `src/app/reset-password/page.tsx` |

## Layouts

| Route | File |
|---|---|
| `/` | `src/app/(app)/layout.tsx` |
| `/reports` | `src/app/(app)/reports/layout.tsx` |
| `/settings` | `src/app/(app)/settings/layout.tsx` |
| `/` | `src/app/layout.tsx` |

## Largest Components

| File | Lines |
|---|---:|
| `src/components/EquipmentPicker.tsx` | 1083 |
| `src/components/TradeBoard.tsx` | 929 |
| `src/components/ActivityTimeline.tsx` | 906 |
| `src/components/ShiftDetailPanel.tsx` | 830 |
| `src/components/BookingDetailsSheet.tsx` | 803 |
| `src/components/onboarding/OnboardingDialog.tsx` | 756 |
| `src/components/ui/sidebar.tsx` | 727 |
| `src/components/booking-wizard/BookingWizard.tsx` | 711 |
| `src/components/ChooseImageModal.tsx` | 676 |
| `src/components/AppShell.tsx` | 601 |
| `src/components/booking-wizard/WizardStep1.tsx` | 565 |
| `src/components/BookingListPage.tsx` | 506 |
| `src/components/shift-detail/ShiftSlotCard.tsx` | 436 |
| `src/components/ui/heatmap.tsx` | 431 |
| `src/components/ui/chart.tsx` | 389 |
| `src/components/FormCombobox.tsx` | 373 |
| `src/components/Sidebar.tsx` | 373 |
| `src/components/PageBreadcrumb.tsx` | 356 |
| `src/components/booking-details/BookingOverview.tsx` | 300 |
| `src/components/QrScanner.tsx` | 288 |
| `src/components/booking-wizard/WizardStep3.tsx` | 263 |
| `src/components/ui/dropdown-menu.tsx` | 258 |
| `src/components/shift-detail/UserAvatarPicker.tsx` | 242 |
| `src/components/ui/context-menu.tsx` | 242 |
| `src/components/shift-detail/CallWindowEditor.tsx` | 236 |
| `src/components/shift-detail/CrewTemplateReviewButton.tsx` | 236 |
| `src/components/booking-list/BookingRow.tsx` | 219 |
| `src/components/booking-list/BookingFilters.tsx` | 202 |
| `src/components/resources/MarkdownReader.tsx` | 202 |
| `src/components/booking-list/types.ts` | 195 |

## Hooks

| File | Lines |
|---|---:|
| `src/hooks/booking-action-copy.ts` | 22 |
| `src/hooks/use-assignment-grid.ts` | 228 |
| `src/hooks/use-current-user.ts` | 28 |
| `src/hooks/use-dashboard-data.ts` | 168 |
| `src/hooks/use-dashboard-filters.ts` | 130 |
| `src/hooks/use-fetch.ts` | 110 |
| `src/hooks/use-form-options.ts` | 32 |
| `src/hooks/use-form-submit.ts` | 188 |
| `src/hooks/use-last-audit.ts` | 74 |
| `src/hooks/use-mobile.ts` | 20 |
| `src/hooks/use-mutate.ts` | 89 |
| `src/hooks/use-scan-submission.ts` | 241 |
| `src/hooks/use-schedule-data.ts` | 524 |
| `src/hooks/use-url-state.ts` | 166 |
| `src/hooks/useBookingActions.ts` | 172 |
| `src/hooks/useBookingDetail.ts` | 71 |
