...
## Change Log
- 2026-04-24: **iOS bookings audit fixes** — closes 1 P0 + 6 P1 from `tasks/audit-bookings-ios.md`. (1) Tab switch now cancels in-flight load via stored `Task` handle so users never see stale data from the previous tab; (2) AC-5 hardening on iOS — toolbar `+` hidden when unauthenticated, requester picker locked to self for STUDENT, BookingDetailView edit pencil gated on role/ownership/state; (3) pagination errors now render an inline Retry row instead of a forever-spinner; (4) Create + Edit sheets show "Discard changes?" confirmation and disable interactive dismiss while submitting; (5) FormCard switched from hardcoded black shadow to `Color.primary` + `.separator` border for dark-mode parity. Files: `ios/Wisconsin/Views/BookingsView.swift`, `BookingDetailView.swift`, `CreateBookingSheet.swift`. Build verification pending in Xcode.
