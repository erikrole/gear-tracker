...
## Change Log
- 2026-04-24: **iOS items audit P2 polish** — purchase price uses `Locale.current.currency`, Reserve completion navigates to the new booking via `BookingRouteId` on the items NavigationStack, and `AssetListBadge` hides requester names from STUDENTs (privacy — STAFF/ADMIN still see them). AC-8 admin actions menu logged as GAP-36 for staff-mobile parity.
