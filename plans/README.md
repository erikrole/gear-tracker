...
## August 2026 Native Operations Utilities

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 062 | Add a read-only Wisconsin Creative menu bar monitor | P1 | M | none | IMPLEMENTED: automated gates, authenticated popover, and notification authorization pass |

Plan 062 is a separate macOS target. It consumes existing authenticated read contracts, preserves kiosk-only custody, and does not authorize deployment, distribution, commit, or push.

## July 2026 Simplification Wave

Generated by the improve skill at commit `189ea5ab` on 2026-07-16. These plans remove duplicated ownership and obsolete compatibility paths without weakening custody, authorization, transaction, audit, or recovery contracts.

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 057 | Centralize server-backed report CSV downloads | P1 | S | active report UI work must be reconciled | BLOCKED: in-scope report pages currently contain user-owned worktree changes |
| 058 | Remove the obsolete hand-written database setup path | P1 | S | none | DONE: branch `codex/058-remove-obsolete-db-setup` (`961892a2`); scoped gates pass, base `verify:docs` has unrelated generated test-count drift |
| 059 | Require an explicit image upload endpoint | P1 | S | none | DONE: branch `codex/059-explicit-image-upload-endpoint` (`f3c301f9`) |
| 060 | Reuse canonical app-time logic in the kiosk dashboard | P1 | S | none | DONE: branch `codex/060-kiosk-app-time-reuse` (`e5e630f1`) |
| 061 | Centralize shift-trade post-commit side effects | P1 | S | none | DONE: branch `codex/061-shift-trade-side-effects` (`e6efe41b`) |

Recommended execution order: 058, 059, 060, and 061 can run independently. Execute 057 only after the current report UI changes are committed or otherwise reconciled, then rerun its drift check against the new HEAD.

### Dependency notes

- 057 is behaviorally independent but overlaps the current dirty report files. Do not execute it from an isolated branch based only on `189ea5ab`, because that would omit the active UI changes.
- 058 must not alter the guarded `db:bootstrap:empty` path or any migration history.
- 060 changes shared time helpers and must land with DST characterization tests.
- 061 shares delivery mechanics only; transaction-owned durable notification writes remain explicit.

### Findings considered and rejected in this wave

- Split `bookings-lifecycle.ts` solely because it is large: rejected. Its mutation boundaries preserve distinct SERIALIZABLE and audit contracts; extract confirmed duplication instead.
- Remove `withAuth` or `withKiosk`: rejected. They centralize authentication, CSRF, password-change, kiosk-session, parameter, and error contracts.
- Inline single-consumer picker hooks: rejected. They isolate abortable search and authoritative conflict recovery from already-large interaction surfaces.
- Remove activity-aware operational polling: rejected. It preserves active operational cadence while pausing hidden, offline, and idle tabs.
- Split `reports.ts` solely by file length: deferred. A file split would improve navigation but not reduce behavior or maintenance materially.
