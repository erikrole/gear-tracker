...
/**
 * Idempotent DDL grouped into batches to stay under the Cloudflare Worker
 * ~50 subrequest limit. All statements use IF NOT EXISTS / IF EXISTS so
 * they're safe to re-run. CREATE TYPE and ADD CONSTRAINT (which lack
 * IF NOT EXISTS in Postgres) are run individually with ignoreError.
 */

// Batch 1: Tables + columns + indexes (all IF NOT EXISTS — safe to batch)
const DDL_BATCH = `
...
// Individual constraint/type steps that may already exist (no IF NOT EXISTS in PG)
const CONSTRAINT_STEPS: { label: string; sql: string }[] = [
 ...
 *
 * Strategy: enums + constraints run individually (may already exist).
 * All DDL with IF NOT EXISTS runs as one batched multi-statement call.
 * Total subrequests: 2 (auth) + 12 (constraints) + 1 (DDL batch) = 15.
 */
export async function POST() {
  try {
    ...
    // Step 1: Create enums + constraints individually (they lack IF NOT EXISTS)
    // Run enums FIRST so the DDL batch can reference them
    ...
    // Step 2: Run all DDL as one batched call
    try {
      await db.$executeRawUnsafe(DDL_BATCH);
      results.push({ label: "DDL batch (tables, columns, indexes)", status: "ok" });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      results.push({ label: "DDL batch (tables, columns, indexes)", status: "error", error: msg });
      return ok({ message: "Migration failed at DDL batch", results });
    }
