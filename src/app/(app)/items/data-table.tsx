            ...
            {table.getRowModel().rows.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() => router.push(row.original.id.startsWith("bulk-") ? `/bulk-inventory?sku=${row.original.id.slice(5)}` : `/items/${row.original.id}`)}
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); router.push(row.original.id.startsWith("bulk-") ? `/bulk-inventory?sku=${row.original.id.slice(5)}` : `/items/${row.original.id}`); } }}
                >
