...
type ListResponse = { data: Reservation[]; total: number; limit: number; offset: number };
...
export default function ReservationsPage() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setCreateError("Failed to create reservation");
      setSubmitting(false);
    }
  }
  ...
  function handleContextMenu(e: React.MouseEvent, reservation: Reservation) {
    e.preventDefault();
    e.stopPropagation();
    setCtxMenu({ x: e.clientX, y: e.clientY, reservation });
  }

  function handleOverflow(e: React.MouseEvent, reservation: Reservation) {
    e.stopPropagation();
    const rect = (e.target as HTMLElement).getBoundingClientRect();
    setCtxMenu({ x: rect.right - 180, y: rect.bottom + 4, reservation });
  }
  ...
  async function handleExtendFromMenu(bookingId: string, days: number) {
    ...
    if (!r) return;
    const extended = new Date(new Date(r.endsAt).getTime() + days * 24 * 60 * 60 * 1000);
    try {
    ...
    <>
          ...
          <div className="create-card-body">
            ...
            <div className="field-compact">
              ...
              <input
                ...
                placeholder={tieToEvent ? "Select an event above..." : "e.g. Game day equipment reservation"}
                required
              />
            </div>
          ...
          <>
            <div className="checkout-table-desktop">
              <table className="data-table">
                ...
                <tbody>
                  {items.map((r) => {
                    const isOverdue = r.status === "BOOKED" && new Date(r.endsAt) < new Date();
                    return (
                      <tr
                        key={r.id}
                      ...
                      >
                        ...
                        <td onClick={(e) => e.stopPropagation()}>
                          <button className="overflow-btn" onClick={(e) => handleOverflow(e, r)}>
      ...
      {ctxMenu && (() => {
        ...
        const ctxAllowed = new Set(
          getAllowedReservationActionsClient(actor, {
            status: ctxMenu.reservation.status,
            requester: ctxMenu.reservation.requester,
            createdBy: ctxMenu.reservation.createdBy,
          })
        );
        return (
          <div
          ...
          >
            <button
              ...
              onClick={() => ctxAction(() => setSelectedBookingId(ctxMenu.reservation.id))}
            >
              View details
            </button>
            ...
            {ctxAllowed.has("extend") && (
              <>
                ...
                <button
                  className="ctx-menu-item"
                  onClick={() => ctxAction(() => handleExtendFromMenu(ctxMenu.reservation.id, 1))}
                >
                  Extend +1 day
                </button>
                <button
                  className="ctx-menu-item"
                  onClick={() => ctxAction(() => handleExtendFromMenu(ctxMenu.reservation.id, 7))}
                >
                  Extend +1 week
                </button>
              </>
            ...
            {ctxAllowed.has("cancel") && (
              <>
                ...
                <button
                  ...
                  onClick={() => ctxAction(() => handleCancelFromMenu(ctxMenu.reservation.id))}
                >
                  Cancel reservation
                </button>
              </>
