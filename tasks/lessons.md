...
- **All-day call-window cleanup must cover local midnight and every producer**: All-day event boundaries may store as local midnight ISO values (for example Central midnight is not UTC midnight), and event detail plus schedule list have separate call-summary/row producers. Hide inherited `default` full-day windows by detecting local midnight-to-local-midnight boundaries in the shared helper, then grep every `formatCallWindow`, `CallWindowEditor`, and `summarizeEffectiveCallWindows` consumer before calling it fixed.
