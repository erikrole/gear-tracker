...
## Session 2026-06-11 (Items list sort/filter audit)

### Patterns
- **A client-side re-sort of server-paginated rows must honor the sort direction**: the items list interleaved bulk-family rows into each page with an unconditional ascending `localeCompare`, silently flipping `order=desc` pages back to ascending while the header arrow showed desc. Any time the client re-orders rows the server already ordered, the comparator must derive direction from the same sort state the request was built from.
- **Sidecar row sources must pass through the same filters as the primary query**: `bulkItems` rode along on page 0 with only location/category/department/q applied, so Retired/Favorites/brand-filtered views interleaved all 21 families anyway. When a response carries a second row collection, audit it against every filter the primary collection supports and exclude it from filters it cannot express.
- **Pagination state from a URL is user input — clamp it against the fetched total**: `?page=9` on a 1-page result rendered "No items match your filters" while matches existed. After each fetch, snap page back into `[0, totalPages-1]` instead of trusting the deep link.
