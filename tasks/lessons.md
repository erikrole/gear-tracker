...
- **Do not use app access role as the only scheduling worker type**: `User.role` controls access, but scheduling has student-specific signals (`gradYear`, student year override, sport assignments, area assignments). A display-only fix still showed students as Staff and routed them into Staff slots because the assignment service, candidate scoring, picker, and schedule rows all defaulted every non-`STUDENT` role to Staff. When staff/student schedule labels are wrong, audit the classifier and API payload end-to-end before changing copy.
