...
- Browser evidence: local seeded admin opened `/schedule` New Event, saw the required-title form-level alert, and hit the newly fixed source-null database constraint during submit; the sheet surfaced the server failure as a form-level alert. Prisma `migrate deploy` failed with the repo's blank schema-engine error before applying the new migration, so the post-create Schedule handoff could not be completed against the current dev database in this run.
