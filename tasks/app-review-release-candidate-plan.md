...
- Build 24 is the current processed App Store Connect TestFlight build. It adds the current app/web booking and item trust contracts to the native passkey, account-security, Schedule, badge, and partial-read reliability work. It is available to Internal QA but intentionally held from External Beta pending physical-device acceptance. The final App Review candidate still needs its own freeze, screenshot recapture, and remaining acceptance proof.
...
- [x] Slice 16: Package and upload Build 24 from commit `cb3261eb`. App Store Connect processed it as `VALID` on 2026-08-09, the complete `en-US` What to Test notes were read back, and Internal QA includes it. External Beta remains gated on installed-binary physical-device acceptance.
...
Build 24 is the current processed Internal QA baseline, but it is not yet the intended App Review submission candidate. Build 24 must be accepted on hardware, matched by fresh screenshots and metadata, and selected in App Store Connect before submission.
...
- [x] Build 24 TestFlight upload, `en-US` What to Test localization, and Internal QA distribution were verified in App Store Connect. The final export contains production APNs, associated domains, `get-task-allow = false`, and Live Activities build 24. External Beta is intentionally pending real-device acceptance.
...
- Next slice or stop: complete the Build 24 Internal QA checklist plus remaining real-device recovery and Live Activities crash checks, then recapture screenshots from that exact archive. Add External Beta only after installed-binary acceptance, and do not submit for review until every proof gate closes.
