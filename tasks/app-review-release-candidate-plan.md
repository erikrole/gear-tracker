...
- Build 23 is the current processed App Store Connect TestFlight build. It includes the current native passkey and account-security hardening plus the latest badge and partial-read reliability work. The final App Review candidate still needs its own freeze, screenshot recapture, and remaining acceptance proof.
...
## Launch Checklist
- [ ] 1. Freeze Build 23 as the App Review candidate. Build 23 is the current processed TestFlight baseline, not yet the final App Review candidate. The exported IPA SHA-256 is `9949429f30e8f44d95a98126341318063ee5cdce9ccd940146a6817a04fa36c9`. Recapture screenshots from this exact binary after the final source freeze and complete the remaining hardware proof before submission.
...
Build 23 is the current processed TestFlight baseline, but it is not yet the intended App Review submission candidate. Build 23 must be frozen from the final source, accepted on hardware, matched by fresh screenshots and metadata, and selected in App Store Connect before submission.
...
- Next slice or stop: complete the remaining real-device recovery and Live Activities crash checks, freeze Build 23 from a reviewed source state, and recapture screenshots from that exact archive. Do not submit for review until those proof gates close.
