...
- [ ] [Polish] **Deferred.** Damage / loss reporting from the kiosk return flow. Web's scan page has `ReportDamageDialog` + `ReportLostDialog` for checkin mode; iOS kiosk return is a "scan-and-complete" surface that doesn't expose those affordances. Project rule per `feedback_ios_vs_web_role.md` ("iOS = day-to-day ops; reports stay on web") suggests the report path stays on the web `/scan?phase=CHECKIN` flow accessible to staff. Revisit if floor staff actually needs to file a damage report from the kiosk.
