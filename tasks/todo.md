...
- [x] **Booking Page Hardening (5-pass)** — Design system (error state → Alert, dead statusBadge cleanup). Data flow (AbortController on detail + list fetches, 401 redirect on all endpoints, differentiated errors, refresh-preserves-data). Resilience (401 on all 4 context menu handlers, success toasts, cancel irreversibility copy, null-safe checkin). UX (manual refresh button + tooltip, extend toast with date). Doc sync. (2026-03-25)
