...
2. **P1 — `POST /api/kiosk/activate` brute-forceable.** 6-digit code, no rate limit. Entire keyspace tries in <30h. `src/app/api/kiosk/activate/route.ts:13`.
...
7. **P1 — `POST /api/notifications/nudge` unscoped + unrate-limited.** Notification spam primitive. `notifications/nudge/route.ts:11`.
...
10. **P2 — Hardening:** ICS token rotation endpoint, force-password-change flag for admin-issued temp passwords, audit IP/UA on password-reset link consumption.
...
- Kiosk session length (currently 30 days) — shorten?
